...
public class BlockDepthsPortal extends BlockBreakable
{
    ...
    public BlockDepthsPortal(final String name) {
        super("eotg:depthsPortal", Material.portal, false);
    ...
    public void onEntityCollidedWithBlock(final World world, final int xPos, final int yPos, final int zPos, final Entity entity) {
        if (entity.ridingEntity == null && entity.riddenByEntity == null && entity instanceof EntityPlayerMP) {
            ...
            final int dimensionID = Config.depths;
            final Block blockFrame = EssenceBlocks.depthsPortalFrame;
    ...
    public void randomDisplayTick(final World world, final int xPos, final int yPos, final int zPos, final Random rand) {
        for (int l = 0; l < 5; ++l) {
            ...
            final EntityDepthsPotalFX var20 = new EntityDepthsPotalFX(world, d0, d2, d3, d4, d5, d6);
    ...
    public static class Size
    {
        ...
        public Size(final World w, final int x, int y, final int z, final int i) {
            ...
            this.w = w;
            this.x = i;
            this.z = BlockDepthsPortal.sides[i][0];
            this.y = BlockDepthsPortal.sides[i][1];
        ...
        protected int setDirection(final int x, final int y, final int z, final int dir) {
            ...
            for (i1 = 0; i1 < 22; ++i1) {
                ...
                if (block2 != EssenceBlocks.depthsPortalFrame) {
                    break;
                }
            }
            final Block block = this.w.getBlock(x + j1 * i1, y, z + k1 * i1);
            return (block == EssenceBlocks.depthsPortalFrame) ? i1 : 0;
        }
        ...
        Label_0272:
            while (this.direc < 21) {
                ...
                for (int j = 0; j < this.sides; ++j) {
                    final int k = this.coords.posX + j * Direction.offsetX[BlockDepthsPortal.sides[this.x][1]];
                    final int l = this.coords.posZ + j * Direction.offsetZ[BlockDepthsPortal.sides[this.x][1]];
                    ...
                    if (j == 0) {
                        block = this.w.getBlock(k + Direction.offsetX[BlockDepthsPortal.sides[this.x][0]], i, l + Direction.offsetZ[BlockDepthsPortal.sides[this.x][0]]);
                        if (block != EssenceBlocks.depthsPortalFrame) {
                            break Label_0272;
                        }
                    }
                    else if (j == this.sides - 1) {
                        block = this.w.getBlock(k + Direction.offsetX[BlockDepthsPortal.sides[this.x][1]], i, l + Direction.offsetZ[BlockDepthsPortal.sides[this.x][1]]);
                        if (block != EssenceBlocks.depthsPortalFrame) {
                            break Label_0272;
                        }
                    }
            ...
            for (int i = 0; i < this.sides; ++i) {
                final int j = this.coords.posX + i * Direction.offsetX[BlockDepthsPortal.sides[this.x][1]];
                final int k = this.coords.posY + this.direc;
                final int l = this.coords.posZ + i * Direction.offsetZ[BlockDepthsPortal.sides[this.x][1]];
                if (this.w.getBlock(j, k, l) != EssenceBlocks.depthsPortalFrame) {
                    this.direc = 0;
                    break;
                }
            }
        ...
        protected boolean getMat(final Block b) {
            return b.getMaterial() == Material.air || b == EssenceBlocks.fire || b == EssenceBlocks.depthsPortal;
        }
        ...
        public void checkDirection() {
            for (int i = 0; i < this.sides; ++i) {
                ...
                for (int l = 0; l < this.direc; ++l) {
                    ...
                    this.w.setBlock(j, i2, k, (Block)EssenceBlocks.depthsPortal, this.x, 2);
                }
            }
        }
    }
