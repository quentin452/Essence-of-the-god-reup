...
public class BlockFrozenPortal extends BlockBreakable
{
    ...
    public BlockFrozenPortal(final String name) {
        super("eotg:frozenPortal", Material.portal, false);
    ...
    public void onEntityCollidedWithBlock(final World world, final int xPos, final int yPos, final int zPos, final Entity entity) {
        if (entity.ridingEntity == null && entity.riddenByEntity == null && entity instanceof EntityPlayerMP) {
            ...
            final int dimensionID = Config.frozen;
            final Block blockFrame = Blocks.snow;
    ...
    public static class Size
    {
        ...
        public Size(final World w, final int x, int y, final int z, final int i) {
            ...
            this.w = w;
            this.x = i;
            this.z = BlockFrozenPortal.sides[i][0];
            this.y = BlockFrozenPortal.sides[i][1];
        ...
        Label_0272:
            while (this.direc < 21) {
                ...
                for (int j = 0; j < this.sides; ++j) {
                    final int k = this.coords.posX + j * Direction.offsetX[BlockFrozenPortal.sides[this.x][1]];
                    final int l = this.coords.posZ + j * Direction.offsetZ[BlockFrozenPortal.sides[this.x][1]];
                    Block block = this.w.getBlock(k, i, l);
                    if (!this.getMat(block)) {
                        break Label_0272;
                    }
                    if (block == EssenceBlocks.frozenPortal) {
                        ++this.value;
                    }
                    if (j == 0) {
                        block = this.w.getBlock(k + Direction.offsetX[BlockFrozenPortal.sides[this.x][0]], i, l + Direction.offsetZ[BlockFrozenPortal.sides[this.x][0]]);
                        if (block != Blocks.snow) {
                            break Label_0272;
                        }
                    }
                    else if (j == this.sides - 1) {
                        block = this.w.getBlock(k + Direction.offsetX[BlockFrozenPortal.sides[this.x][1]], i, l + Direction.offsetZ[BlockFrozenPortal.sides[this.x][1]]);
                        if (block != Blocks.snow) {
                            break Label_0272;
                        }
                    }
            ...
            for (int i = 0; i < this.sides; ++i) {
                final int j = this.coords.posX + i * Direction.offsetX[BlockFrozenPortal.sides[this.x][1]];
                final int k = this.coords.posY + this.direc;
                final int l = this.coords.posZ + i * Direction.offsetZ[BlockFrozenPortal.sides[this.x][1]];
                if (this.w.getBlock(j, k, l) != Blocks.snow) {
                    this.direc = 0;
                    break;
                }
            }
        ...
        protected boolean getMat(final Block b) {
            return b.getMaterial() == Material.air || b == EssenceBlocks.fire || b == EssenceBlocks.frozenPortal;
        }
        ...
        public void checkDirection() {
            for (int i = 0; i < this.sides; ++i) {
                ...
                for (int l = 0; l < this.direc; ++l) {
                    ...
                    this.w.setBlock(j, i2, k, (Block)EssenceBlocks.frozenPortal, this.x, 2);
                }
            }
        }
    }
