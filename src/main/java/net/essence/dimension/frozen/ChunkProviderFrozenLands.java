...
import net.essence.dimension.frozen.gen.*;
...
public class ChunkProviderFrozenLands implements IChunkProvider
{
    ...
    private double[] gen4;
    private int[][] ia;
    private ArrayList<WorldGenerator> gens;
    private ArrayList<WorldGenerator> rare;
    
    public ChunkProviderFrozenLands(final World par1World, final long par2) {
        this.stoneNoise = new double[256];
        this.ia = new int[32][32];
        this.worldObj = par1World;
        this.type = par1World.getWorldInfo().getTerrainType();
        this.rand = new Random(par2);
        (this.gens = new ArrayList<WorldGenerator>(0)).add(new WorldGenCandyCane());
        (this.rare = new ArrayList<WorldGenerator>(2)).add(new WorldGenSantasWorkshop());
        this.rare.add(new WorldGenPresent());
                    ...
                    for (int l3 = 0; l3 < 8; ++l3) {
                        ...
                        for (int i4 = 0; i4 < 4; ++i4) {
                            ...
                            for (int k4 = 0; k4 < 4; ++k4) {
                                if ((d17 += d16) > 0.0) {
                                    b[j4 += short1] = EssenceBlocks.frozenStone;
                                }
                                else if (k3 * 8 + l3 < 62) {
                                    b[j4 += short1] = EssenceBlocks.frozenDirt;
                                }
                                else if (k3 * 8 + l3 < b2) {
                                    b[j4 += short1] = EssenceBlocks.frozenGrass;
                                }
                                else {
                                    b[j4 += short1] = null;
                                }
        ...
        for (int l2 = 255; l2 >= 0; --l2) {
            ...
            if (l2 <= 0 + rand.nextInt(5)) {
                ...
                if (block3 != null && block3.getMaterial() != Material.air) {
                    if (block3 == EssenceBlocks.frozenStone) {
                        if (k == -1) {
                            if (l <= 0) {
                                block = null;
                                b2 = 0;
                                block2 = EssenceBlocks.frozenStone;
                            }
                            else if (l2 >= 59 && l2 <= 64) {
                                block = b.topBlock;
                            ...
                            if (l2 >= 62) {
                                ...
                                block = null;
                                block2 = EssenceBlocks.frozenStone;
                                blocks[i3] = EssenceBlocks.frozenStone;
                            }
                            else {
                                blocks[i3] = block2;
                            }
                            ...
                            --k;
                            blocks[i3] = block2;
                            if (k == 0 && block2 == EssenceBlocks.frozenStone) {
            ...
            for (int k1 = 0; k1 < 5; ++k1) {
                ...
                for (int l2 = -b0; l2 <= b0; ++l2) {
                    for (int i2 = -b0; i2 <= b0; ++i2) {
                        ...
                        final float f4 = biomegenbase2.rootHeight;
                        final float f5 = biomegenbase2.heightVariation;
    ...
    public void populate(final IChunkProvider par1IChunkProvider, final int chunkX, final int chunkZ) {
        ...
        if (this.rand.nextInt(100) == 0) {
            final int y = this.worldObj.getHeightValue(x1, z1);
            final int x2 = x1 + this.rand.nextInt(16);
            final int z2 = z1 + this.rand.nextInt(16);
            if (this.worldObj.getBlock(x2, y, z2) == Blocks.air && this.worldObj.getBlock(x2, y - 1, z2) == EssenceBlocks.frozenGrass && this.worldObj.getBlock(x2, y + 1, z2) == Blocks.air) {
                this.rare.get(this.rand.nextInt(this.gens.size())).generate(this.worldObj, this.rand, x2, y - 1, z2);
            }
        }
        for (int times = 0; times < 2; ++times) {
            ...
            final int z2 = z1 + this.rand.nextInt(16);
            if (this.worldObj.getBlock(x2, y, z2) == Blocks.air && this.worldObj.getBlock(x2, y - 1, z2) == EssenceBlocks.frozenGrass && this.worldObj.getBlock(x2, y + 1, z2) == Blocks.air) {
                this.gens.get(this.rand.nextInt(this.gens.size())).generate(this.worldObj, this.rand, x2, y - 1, z2);
            }
        }
    }
