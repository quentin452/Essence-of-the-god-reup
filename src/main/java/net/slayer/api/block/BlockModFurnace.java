...
public abstract class BlockModFurnace extends BlockContainer
{
    ...
    public BlockModFurnace(final String name, final boolean act, final int GuiID) {
        super(Material.rock);
    ...
    public boolean onBlockActivated(final World w, final int x, final int y, final int z, final EntityPlayer p, final int i, final float j, final float k, final float f) {
        final TileEntityModFurnace furnace = (TileEntityModFurnace)w.getTileEntity(x, y, z);
        if (!w.isRemote && furnace != null && !p.isSneaking()) {
            p.openGui((Object)Essence.instance, this.guiID, w, x, y, z);
        }
        return false;
    }
    
    public static void updateActiveStates(final boolean active, final World w, final int x, final int y, final int z) {
    }
    ...
    public void onBlockPlacedBy(final World w, final int x, final int y, final int z, final EntityLivingBase living, final ItemStack item) {
        ...
        if (item.hasDisplayName()) {
            ((TileEntityModFurnace)w.getTileEntity(x, y, z)).setCustomName(item.getDisplayName());
        }
    }
    
    public void breakBlock(final World w, final int x, final int y, final int z, final Block b, final int meta) {
        if (!BlockModFurnace.keepInventory) {
            final TileEntityModFurnace entity = (TileEntityModFurnace)w.getTileEntity(x, y, z);
            if (entity != null) {
    ...
    public void randomDisplayTick(final World w, final int x, final int y, final int z, final Random r) {
        if (this.active) {
            ...
            final float f2 = y + 0.0f + r.nextFloat() * 6.0f / 16.0f;
            final float f3 = z + 0.5f;
            final float f4 = 0.52f;
            final float f5 = r.nextFloat() * 0.6f - 0.3f;
            if (meta == 4) {
